package qea.benchmark.rovers.junitrv;
...
public class JunitRVDoEval extends DoEval<String> {
	...
	private static Monitor propositional_resourceLifecycle = new FLTL4Monitor(
			Always(and(
					and(and(implies(request, X(or(deny, grant))),
							implies(deny, X(or(request, request)))),
							and(implies(grant, X(or(cancel, rescind))),
									implies(cancel, X(or(request, request))))),
					implies(rescind, X(or(cancel, rescind))))));

	// SETUP for first-order resource lifecycle

	private static final SMTPredicate id = new SMTPredicate("p",
			new Serializable() {
				public SMTExpression<Void> p(JunitRVDoWork work, Object o) {
					return new SMTOperator<>("eq",
							ImmutableList.<SMTExpression<Void>> of(
									new SMTVariable<Void>("x"),
									new SMTInteger<Void>(System
											.identityHashCode(o))));
				}
			});
	private static final Proposition<SMTExpression<Property>> propId = new Proposition<SMTExpression<Property>>(
			new SMTSymbol<Property>(id));

	private static final LTL<SMTExpression<Property>> request_f = new AND<>(
			ImmutableList.<LTL<SMTExpression<Property>>> of(propId,
					new Proposition<SMTExpression<Property>>(
							new SMTSymbol<Property>(request))));
	private static final LTL<SMTExpression<Property>> grant_f = new AND<>(
			ImmutableList.<LTL<SMTExpression<Property>>> of(propId,
					new Proposition<SMTExpression<Property>>(
							new SMTSymbol<Property>(grant))));
	private static final LTL<SMTExpression<Property>> deny_f = new AND<>(
			ImmutableList.<LTL<SMTExpression<Property>>> of(propId,
					new Proposition<SMTExpression<Property>>(
							new SMTSymbol<Property>(deny))));
	private static final LTL<SMTExpression<Property>> cancel_f = new AND<>(
			ImmutableList.<LTL<SMTExpression<Property>>> of(propId,
					new Proposition<SMTExpression<Property>>(
							new SMTSymbol<Property>(cancel))));
	private static final LTL<SMTExpression<Property>> rescind_f = new AND<>(
			ImmutableList.<LTL<SMTExpression<Property>>> of(propId,
					new Proposition<SMTExpression<Property>>(
							new SMTSymbol<Property>(rescind))));

	private static final LTL<SMTExpression<Property>> request_next = new OR<>(
			ImmutableList.of(new NOT<>(request_f), new X<>(new OR<>(
					ImmutableList.of(deny_f, grant_f)))));
	private static final LTL<SMTExpression<Property>> grant_next = new OR<>(
			ImmutableList.of(new NOT<>(grant_f),
					new X<>(new OR<>(ImmutableList.of(cancel_f, rescind_f)))));
	private static final LTL<SMTExpression<Property>> deny_next = new OR<>(
			ImmutableList.of(new NOT<>(deny_f),
					new X<>(new OR<>(ImmutableList.of(request_f)))));
	private static final LTL<SMTExpression<Property>> cancel_next = new OR<>(
			ImmutableList.of(new NOT<>(cancel_f), new X<>(new OR<>(
					ImmutableList.of(request_f)))));
	private static final LTL<SMTExpression<Property>> rescind_next = new OR<>(
			ImmutableList.of(new NOT<>(rescind_f), new X<>(new OR<>(
					ImmutableList.of(cancel_f, rescind_f)))));

	private static final AND<SMTExpression<Property>> all_together = new AND<>(
			ImmutableList.of(request_next, grant_next, deny_next, cancel_next,
					rescind_next));
