...
public class QEABuilder {
	...
	/*
	 * To create a QEA we need
	 *  - a set of transitions
	 *  - a set of final stats
	 *  - a quanfication list
	 *  
	...
	private static class TempTransition{
		...
		public int[] var_args(){
			...
			for(int i=0;i<event_args.length;i++){
				...
				if(vargs[i]==0) throw new RuntimeException("Do not yet support values in transitions");
			}
			return vargs;
		}
	...
	public QEA make(){
		...
		// Propositional first
		if(quants.size()==0) return makeProp(states,events);
		// Single quantifier next
		if(quants.size()==1) return makeSingle(states,events);
	...
	private QEA makeProp(int states, int events){
		if(noFreeVariables()){
			if(isEventDeterministic()){
				SimpleDetQEA qea = new SimpleDetQEA(states,events,1,NONE);
				for(TempTransition t : transitions) qea.addTransition(t.start,t.event_name,t.end);
				for(Integer s : finalstates) qea.setStateAsFinal(s);
				return qea;
			}
			else{
				SimpleNonDetQEA qea = new SimpleNonDetQEA(states,events,1,NONE);
				...
				for(TempTransition t : transitions) {
					...
					if(this_state==null) actual_trans[t.start] = new HashMap<Integer,Set<Integer>>();
					this_state = actual_trans[t.start];
					
					Set<Integer> nextstates = this_state.get(t.event_name);
					if(nextstates==null){ 
						nextstates = new HashSet<Integer>(); this_state.put(t.event_name, nextstates);
					}
					nextstates.add(t.end);
				}
				for(int i=0;i<actual_trans.length;i++){
					if(actual_trans[i]!=null)
						for(Map.Entry<Integer,Set<Integer>> entry : actual_trans[i].entrySet()){
								int[] nextstates = new int[entry.getValue().size()];
								int j=0;
								for(Integer s : entry.getValue()){nextstates[j]=s;j++;}
	...
	private QEA makeSingle(int states, int events){
		Quantification q;
		if(quants.get(0).universal) q = FORALL; else q = EXISTS;
		
		if(noFreeVariables()){
			if(isEventDeterministic()){
				SimpleDetQEA qea = new SimpleDetQEA(states,events,1,q);
				for(TempTransition t : transitions) qea.addTransition(t.start,t.event_name,t.end);
				for(Integer s : finalstates) qea.setStateAsFinal(s);
				return qea;
			}
			else{				
				SimpleNonDetQEA qea = new SimpleNonDetQEA(states,events,1,q);
				Map<Integer,Set<Integer>>[] actual_trans = new Map[states];
				for(TempTransition t : transitions) {
					Map<Integer,Set<Integer>> this_state = actual_trans[t.start];
					if(this_state==null) actual_trans[t.start] = new HashMap<Integer,Set<Integer>>();
			...
			if(singleQvarFirst()){
				if(isEventDeterministic()){
					NonSimpleDetQEA qea = new NonSimpleDetQEA(states,events,1,q,frees);
				
					for(TempTransition t : transitions){
						Transition trans = new TransitionImpl(t.var_args(),t.end);
					...
					NonSimpleNonDetQEA qea = new NonSimpleNonDetQEA(states,events,1,q,frees);
					Map<Integer,Set<TransitionImpl>>[] actual_trans = new Map[states];
					for(TempTransition t : transitions) {
						Map<Integer,Set<TransitionImpl>> this_state = actual_trans[t.start];
						if(this_state==null) actual_trans[t.start] = new HashMap<Integer,Set<TransitionImpl>>();
						this_state = actual_trans[t.start];
						
						Set<TransitionImpl> nextstates = this_state.get(t.event_name);
						if(nextstates==null){ 
					...
					for(int i=0;i<actual_trans.length;i++){
						if(actual_trans[i]!=null)
							for(Map.Entry<Integer,Set<TransitionImpl>> entry : actual_trans[i].entrySet()){
									TransitionImpl[] nextstates = new TransitionImpl[entry.getValue().size()];
									int j=0;
									for(TransitionImpl s : entry.getValue()){nextstates[j]=s;j++;}
									qea.addTransitions(i,((int)entry.getKey()),nextstates);
							}
					}
					for(Integer s : finalstates) qea.setStateAsFinal(s);	
				...
				if(isEventDeterministic()){
					NonSimpleDetGenQEA qea = new NonSimpleDetGenQEA(states,events,1,q,frees);
					...
					NonSimpleNonDetGenQEA qea = new NonSimpleNonDetGenQEA(states,events,1,q,frees);
					Map<Integer,Set<TransitionImpl>>[] actual_trans = new Map[states];
					for(TempTransition t : transitions) {
						Map<Integer,Set<TransitionImpl>> this_state = actual_trans[t.start];
						if(this_state==null) actual_trans[t.start] = new HashMap<Integer,Set<TransitionImpl>>();
	...
	private int countStates(){
		...
		for(TempTransition t : transitions){ states.add(t.start); states.add(t.end);}
		return states.size();
	}
	private int countEvents(){
		Set<Integer> events = new HashSet<Integer>();
		for(TempTransition t : transitions){ events.add(t.event_name);}
		return events.size();		
	}
	private int countFreeVars(){
		Set<Integer> qvars = new HashSet<Integer>();
		for(Quant q : quants) qvars.add(q.var);
	...
	/*
	 * Check that all transitions have a start, end and event name
	 * And that all quantifications have a var
	 */
	private boolean wellFormed(){
		for(TempTransition trans : transitions){
			if(trans.start == -1) return false;
			if(trans.end == -1) return false;
			if(trans.event_name == -1) return false;
			if(trans.event_args!=null)
				for(int i=0;i<trans.event_args.length;i++)
					if(trans.event_args[i]==null || 
					(trans.event_args[i].val==null && trans.event_args[i].var == -1)) return false;
		}		
		for(Quant q : quants) if(q.var == -1) return false;
		return true;
	}
	
	/*
	 * Get the quantified variables
	 * Check that all transitions that have arguments either use a value or a quantified variable
	 */
		...
		for(TempTransition t : transitions){
			if(t.event_args!=null)
				for(int i=0;i<t.event_args.length;i++)
					...
					!(qvars.contains(t.event_args[i].var))) return false;
		}		
		return true;
	}
	/*
	...
	private boolean isEventDeterministic(){
		...
		for(TempTransition t : transitions){
			if(start_trans.put(t.start,t.event_name) != null) return false;
		}		
		return true;
	}
	/*
	 * Check that there is a single quantified variable appearing only in the first position
	 */
	private boolean singleQvarFirst(){
		if(quants.size()!=1) return false;
		int var = quants.get(0).var;
		for(TempTransition t : transitions){
			if(t.event_args[0].var==var) continue;
			for(int i=1;i<t.event_args.length;i++)
				if(t.event_args[i].var==var) return false;
		}					
		return true;
	}
	...
	public void addFinalStates(int... states) {
		for(int i=0;i<states.length;i++) finalstates.add(states[i]);
	}
