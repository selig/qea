...
public class Interface {
	...
	public void USER_closeAccount(Integer uid, Integer sid,
	...
	public void USER_depositFromExternal(Integer uid, Integer sid,
	...
	public boolean USER_payToExternal(Integer uid, Integer sid,
			String from_account_number, double amount) {
		UserInfo u = ts.getUserInfo(uid);
		UserSession s = u.getSession(sid);
		...
		if (u.getAccount(from_account_number).getBalance() >= amount) {
			s.log("Payment of $" + amount + " from account <"
					+ from_account_number + ">");
			u.withdrawFrom(from_account_number, total_amount);
			return true;
		}
		s.log("FAILED (not enough funds): Payment of $" + amount
				+ " from account <" + from_account_number + ">");
		return false;
	}

	// * Transfer money to another user's account - charges apply
	public boolean USER_transferToOtherAccount(Integer from_uid, Integer sid,
			String from_account_number, Integer to_uid,
			String to_account_number, double amount) {
		...
		if (from_u.getAccount(from_account_number).getBalance() >= total_amount) {
			...
			s.log("Payment of $" + amount + " from account <"
					+ from_account_number + "> to account " + "<"
					+ to_account_number + " of user " + to_uid);
			return true;
		}
		s.log("FAILED (not enough funds): " + "Payment of $" + amount
				+ " from account <" + from_account_number + "> to account "
				+ "<" + to_account_number + " of user " + to_uid);
		return false;
	}

	// * Transfer money across own accounts - charges do not apply
	public boolean USER_transferOwnAccounts(Integer uid, Integer sid,
			String from_account_number, String to_account_number, double amount) {
		UserInfo u = ts.getUserInfo(uid);
		UserSession s = u.getSession(sid);
		UserAccount from_a = ts.getUserInfo(uid)
				.getAccount(from_account_number), to_a = ts.getUserInfo(uid)
				.getAccount(to_account_number);

		if (from_a.getBalance() >= amount) {
			from_a.withdraw(amount);
			to_a.deposit(amount);
			s.log("Transfer of $" + amount + " from account <"
					+ from_account_number + "> to own account <"
					+ to_account_number);
			return true;
		}
		s.log("FAILED (not enough funds)" + "Transfer of $" + amount
				+ " from account <" + from_account_number
