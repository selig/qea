...
abstract class DoWork<S>{
  ...
  public void dowork(String name, int[] args){
      if(name.equals("IncreasingCommand")) work_for_IncreasingCommand(args[0]); 
      if(name.equals("ResourceLifecycle")) work_for_ResourceLifecycle(args[0],args[1]); 
      if(name.equals("ExactlyOneSuccess")) work_for_ExactlyOneSuccess(args[0]); 
      if(name.equals("AcknowledgeCommands")) work_for_AcknowledgeCommands(args[0]); 
      if(name.equals("NestedCommand")) work_for_NestedCommand(args[0],args[1],args[2]);
      if(name.equals("GrantCancel")) work_for_GrantRelease(args[0],args[1],args[2]); 
      if(name.equals("ReleaseResource"))  work_for_ReleaseResource(args[0],args[1],args[2]); 
      if(name.equals("RespectConflicts")) work_for_RespectConflicts(args[0],args[1],args[2]); 
      if(name.equals("ExistsSatellite")) work_for_ExistsSatellite(args[0],args[1]); 
      if(name.equals("ExistsLeader")) work_for_ExistsLeader(args[0]); 
      if(name.equals("MessageHashCorrectInvInt")) work_for_MessageHashCorrectInvInt(args[0],args[1]);
  ...
  public void work_for_ResourceLifecycle(int r,int u){
    ...
    for(int i=0;i<r;i++) ros[i] = new Object();
    ...
    for(int i =0;i<u;i++){
      ...
      int sp = -1;
      switch(s){
        case 0 : request(ros[res]); sp = 1;break;
        case 1 : if(rand.nextBoolean()){ grant(ros[res]); sp=2;} else{ deny(ros[res]); sp=0;};break;
        case 2 : if(rand.nextBoolean()){ rescind(ros[res]);sp=2;} else{ cancel(ros[res]);sp=0;};break;
      }
      resources[res]=sp;
    }
  ...
  public void work_for_ExactlyOneSuccess(int c){
    ...
    for(int i=0;i<c;i++) cos[i] = new Object();
  ...
  private class AC_tosuc_Entry {
          ...
          public AC_tosuc_Entry(int a, int b, Object c, Object d){
             com=a;time=b;node=c;payload=d;
          }
  }

  public void work_for_AcknowledgeCommands(int c){

    Object[] cos = new Object[c]; for(int i=0;i<c;i++) cos[i]=new Object();
    Set<Object> extra_c = new HashSet<Object>();
    set_ack_timeout(100);

    Queue<AC_tosuc_Entry> to_suc = new LinkedList<AC_tosuc_Entry>();  //Set[(Int,Object,Object,Int)] = Set()

    int c_made = 0; int clock=0;

    Random rand = new Random();
    while(c_made<c || !to_suc.isEmpty()){
      if(c_made==c || (!to_suc.isEmpty() && rand.nextBoolean())){
        AC_tosuc_Entry entry = to_suc.remove();    // (com,n,p,t) = to_suc.head
        if(rand.nextBoolean()){
        ...
        AC_tosuc_Entry entry = new AC_tosuc_Entry(c_made,clock,node,payload);
        to_suc.add(entry);
  ...
  public void work_for_NestedCommand(int depth, int breadth, int reps){
    for(int i=0;i<reps;i++) nested_do_rep(depth,breadth);
  }
  
  
  public void work_for_GrantRelease(int t,int r,int u){
    
    Object[] tos = new Object[t]; for(int i=0;i<t;i++) tos[i]=new Object();
    Object[] ros = new Object[r]; for(int i=0;i<r;i++) ros[i]=new Object();
    // which task owns the resource -1 means not owned
    int[] owning = new int[r];
    for(int i=0;i<r;i++) owning[i]= -1;
  ...
  public void work_for_ReleaseResource(int t,int r,int e){
    ...
    for(int i=0;i<=t;i++) t_to_c.put(i, -1);

    Map<Integer,Queue<Integer>> t_to_r = new HashMap<Integer,Queue<Integer>>();
    for(int i=0;i<=t;i++) t_to_r.put(i, new LinkedList<Integer>());

    int[] resources = new int[r];
    for(int i=0;i<r;i++) resources[i] = -1;
    ...
    for(int i=0;i<e;i++){
      ...
      if(com == -1){
        ...
        else{ if(r_used==r || (!res.isEmpty() && rand.nextBoolean())){
          // remove resource
          int rr = res.remove();
          resources[rr]= -1;
      ...
      // groups of things that are not in conflict with each other
		...
		for(int i=0;i<r;i++){
			...
			for(int j=0;j<r;j++){
			  ...
			  group.add(new Object(){  public String toString(){ return label;}});
			}
			groups.add(group);
		}
		...
		for(int i=0;i<s;i++){
			...
			for(int j=0;j<r;j++)
				if(granted[j]) cancel(group.get(j));
		}    
  }
  ...
  public void work_for_ExistsSatellite(int s,int r){
    ...
    Object[] sos = new Object[s]; for(int i=0;i<s;i++) sos[i]=new Object();
    Object[] ros = new Object[r]; for(int i=0;i<r;i++) ros[i]=new Object();
    ...
    for(int i=0;i<s;i++) sats_p.add(i);
    ...
    while(!sats_p.isEmpty() || !sats_r.isEmpty()){
      ...
      if((!sats_p.isEmpty() && rand.nextBoolean()) || sats_r.isEmpty()){
              Integer sat = sats_p.get(rand.nextInt(sats_p.size())); // important that it's an object for list remove method
	      ...
	      if(temp_q==null){ temp_q = new LinkedList(); temp_q.add(rr);}
	      ...
	      if(res.size()==1){
              ...
              Integer sat = sats_r.get(rand.nextInt(sats_r.size())); // important that it's an object for list remove method
  ...
  public static void work_for_ExistsLeader(int r){
    ...
    for(int res=0;res<r;res++){
      ...
      if(!o_res.isEmpty()) map_p.put(res,o_res);
    }
    
    int leader = rand.nextInt(r);
    Queue<Integer> o_res = new LinkedList<Integer>();
    for(int i=0;i<r;i++){ o_res.add(i); }
    ...
    for(int i : map_p.keySet()) res_p.add(i);

    List<Integer> res_a = new ArrayList<Integer>();

    System.err.println("I got part way through ExistsLeader then gave up translating as we probs won't use it");
    /*
    while(!res_p.isEmpty() || !res_a.isEmpty()){       
      ...
      if((!res_p.isEmpty() && rand.nextBoolean()) || res_a.isEmpty()){
        // send ping
	      ...
	      val res = res_p(rand.nextInt(res_p.length))
	      val o_res = map_p(res)
	      val rr = o_res.head
	      m.step(('ping,List(ros(res),ros(rr))))
	      res_a +:= rr
	      map_a += (rr -> (map_a.getOrElse(rr,Set[Int]())+res))
	      
	      if(o_res.size==1){
	    	  map_p -= res	 
	    	  res_p = res_p.filter{_!=res}
	      }
	      else
	    	  map_p += ((res -> o_res.tail))
      }
      else{
        // send ack
	      val res = res_a(rand.nextInt(res_a.length))
	      val o_res =map_a(res)
	      val rr = o_res.head
	      m.step(('ack,List(ros(res),ros(rr))))
	      
	      if(o_res.size==1){
	    	  map_a -= res	
	    	  res_a = res_a.filter{_!=res}
	      }
	      else
  ...
  private static class mhc_Entry{
          ...
          public mhc_Entry(Object a, Object b, int c){
                 sender=a;receiver=b;hash=c;
          }
  }

  public void work_for_MessageHashCorrectInvInt(int x,int ms){
    
    Object[] xos = new Object[x]; for(int i=0;i<x;i++) xos[i]=new Object();
