...
public abstract class Abstr_QVarN_QEA extends QEA {
	...
	/*
	 * The underlying structure is either Det or NonDet but the contents can
	 * either use free variables or not
	 * 
	...
	/*
	 * If it is not normal we need to add sub-bindings for all bindings as *all*
	 * bindings must be generated, even if they are associated with the empty
	 * projection
	 */
	...
	public static class QEntry {
		...
		public String toString() {
			return quantification + " x_" + variable
					+ (guard != null ? guard : "");
		}
	...
	public QBindingImpl[] makeBindings(int eventName, Object[] args) {
		...
		/*
		 * for(int e = 1; e<e_sigs.length;e++){ System.out.println(e); for(int[]
		...
		for (int i = 0; i < sigs.length; i++) {
			...
			for (int j = 0; j < use_next; j++) {
				Object val = bs[j];
				if (val != null && val.equals(qbinding)) {
					repeated = true;
				}
			}
			if (!repeated) {
				...
				for (QBindingImpl b2 : bs) {
					if (!b1.equals(b2)) {
						for (int q = 1; q < quantifiedVariableCount; q++) {
							...
							if (v1 != null || v2 != null) {
								if (v1 == null) {
									QBindingImpl newone = (QBindingImpl) b1
											.copy();
									newone.setValue(-q, v2);
									boolean repeated = false;
									for (QBindingImpl other : bs) {
										if (other.equals(newone)) {
											repeated = true;
										}
									}
									...
									if (!repeated) {
										changing = true;
										add = ArrayUtil.resize(add,
												add.length + 1);
										add[add.length - 1] = newone;
									}
								} else if (v2 == null) {
									QBindingImpl newone = (QBindingImpl) b2
											.copy();
