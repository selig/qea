 ...
 * @author Giles Reger
 */
public class Incr_QVarN_NoFVar_Det_QEAMonitor extends
		Abstr_Incr_QVarN_QEAMonitor<QVarN_Det_QEA> {
	...
	public Verdict step(int eventName) {
		...
		case NONE:
			...
			break;
		case UNSAFE_LAZY:
			new_mapping = new OurWeakHashMap<QBindingImpl, Integer>(
					mapping.size());
			break;
		default:
			throw new RuntimeException("Garbage mode " + garbage_mode
					+ " not currently supported");
		}
		...
		for (Map.Entry<QBindingImpl, Integer> entry : mapping.entrySet()) {
			...
			Integer next_state = qea.getNextConfig(binding, previous_state,
					eventName, dummyArgs);
			if (next_state != null) {
	...
	protected void processBinding(int eventName, Object[] args,
		...
		if (DEBUG) {
			System.err.println("this_use_red: " + this_use_red);
		}
		...
		// TODO - does it fully reflect the redundancy elimination discussed in
		// paper?
		//
		if (!this_use_red || could_leave[previous_state][eventName]) {
			QBindingImpl[] bs = qea.makeBindings(eventName, args);
			if (DEBUG) {
				System.err.println("bs: " + Arrays.toString(bs));
			}
			for (QBindingImpl from_binding : bs) {
				...
				if (binding.consistentWith(from_binding)) {
					...
					if (!mapping.containsKey(ext)) {
						...
						if (DEBUG) {
							System.err.println("Adding new " + ext + " with "
									+ next_state);
						}
						if (next_state != null) {
		...
		if (!has_q_blanks) {
			Integer next_state = qea.getNextConfig(binding, previous_state,
					eventName, args);
			if (DEBUG) {
				System.err.println("Updating " + binding + " with "
						+ next_state);
			}
			if (next_state != null) {
	...
	protected int removeStrongBindings() {
		...
		if (DEBUG) {
			System.err.println("Removing strong: " + strong_bindings);
		}
